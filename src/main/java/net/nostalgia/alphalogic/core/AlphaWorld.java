...
public class AlphaWorld {
  ...
  public int getBlockId(int x, int y, int z) {
    if (y >= this.mcLevel.getMinY() && y < this.mcLevel.getMaxY()) {
      BlockPos pos = new BlockPos(x, y, z);
      BlockState state = this.mcLevel.getBlockState(pos);
      if (state.isAir()) {
        return 0;
        ...
        return 51;
      } else if (state.is(AlphaBlocks.ALPHA_ICE) || state.is(Blocks.ICE)) {
        return 79;
      } else if (state.is(AlphaBlocks.ALPHA_CACTUS) || state.is(Blocks.CACTUS)) {
        return 81;
      } else {
        return state.getCollisionShape(this.mcLevel, pos).isEmpty() ? 0 : 1;
      }
    } else {
      return 0;
    }
  ...
  public List<AlphaAABB> getCollidingBoundingBoxes(AlphaEntity entity, AlphaAABB aabb) {
    ...
    for (int x = minX; x < maxX; x++) {
      for (int y = minY - 1; y < maxY; y++) {
        for (int z = minZ; z < maxZ; z++) {
          ...
          if (blockId > 0 && blockId != 8 && blockId != 9 && blockId != 10 && blockId != 11 && blockId != 50 && blockId != 51) {
            if (blockId == 81) {
              float f = 0.0625F;
    ...
    AABB mcBox = new AABB(aabb.minX, aabb.minY, aabb.minZ, aabb.maxX, aabb.maxY, aabb.maxZ);

    for (Entity modernEntity : this.mcLevel
      .getEntities((Entity)null, mcBox, e -> e.canBeCollidedWith((Entity)null) || e.isPushable() || e instanceof AlphaBoatEntity)) {
      if (!(modernEntity instanceof Player)) {
        AABB boundingBox = modernEntity.getBoundingBox();
  ...
  public void tickUnits() {
    for (int i = 0; i < this.loadedEntities.size(); i++) {
      AlphaEntity e = this.loadedEntities.get(i);
      if (!e.isDead) {
        e.onUpdate();
      } else {
        this.loadedEntities.remove(i--);
      }
    }
  ...
  public void pushEntities() {
    for (int i = 0; i < this.loadedEntities.size(); i++) {
      AlphaEntity e1 = this.loadedEntities.get(i);
      if (!e1.isDead) {
        for (int j = i + 1; j < this.loadedEntities.size(); j++) {
          AlphaEntity e2 = this.loadedEntities.get(j);
          if (!e2.isDead && e1.riddenByEntity != e2 && e1.ridingEntity != e2) {
  ...
  public boolean handleMaterialAcceleration(AlphaAABB aabb, int materialId, AlphaEntity entity) {
      ...
      for (int y = minY; y < maxY; y++) {
        for (int z = minZ; z < maxZ; z++) {
          ...
          if (blockId == materialId || materialId == 8 && blockId == 9 || materialId == 10 && blockId == 11) {
            BlockPos pos = new BlockPos(x, y, z);
            FluidState fs = this.mcLevel.getFluidState(pos);
            double height = y + 1 - 0.1111111111111111;
            if (maxY >= height) {
              inMaterial = true;
              if (materialId == 8 || materialId == 10) {
                Vec3 modernFlow = fs.getFlow(this.mcLevel, pos);
                pushVec.x = pushVec.x + modernFlow.x;
                pushVec.y = pushVec.y + modernFlow.y;
                pushVec.z = pushVec.z + modernFlow.z;
              }
            }
          }
        }
    ...
    if (pushVec.length() > 0.0 && inMaterial) {
      ...
      entity.motionX = entity.motionX + pushVec.x * strength;
      entity.motionY = entity.motionY + pushVec.y * strength;
      entity.motionZ = entity.motionZ + pushVec.z * strength;
    }
