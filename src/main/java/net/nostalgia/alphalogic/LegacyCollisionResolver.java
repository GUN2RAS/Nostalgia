...
public class LegacyCollisionResolver {
  ...
  public Vec3 resolveMovement(Vec3 requestedMotion) {
    if (this.level == null) {
      ...
      List<AABB> colliders = this.gatherNearbyColliders(this.entityBounds.expandTowards(mx, my, mz));
      if (colliders.isEmpty()) {
        ...
        for (AABB box : colliders) {
          adjustedY = this.clipYCollide(this.entityBounds, box, adjustedY);
        }

        AABB movedY = this.entityBounds.move(0.0, adjustedY, 0.0);
        double adjustedX = mx;

        for (AABB box : colliders) {
          adjustedX = this.clipXCollide(movedY, box, adjustedX);
        }
        ...
        for (AABB box : colliders) {
          adjustedZ = this.clipZCollide(movedXY, box, adjustedZ);
        }
  ...
  private double clipYCollide(AABB entity, AABB block, double motion) {
    if (entity.maxX > block.minX && entity.minX < block.maxX && entity.maxZ > block.minZ && entity.minZ < block.maxZ) {
      if (motion > 0.0 && entity.maxY <= block.minY + 1.0E-7) {
        double gap = block.minY - entity.maxY;
        if (gap < motion) {
          motion = gap;
        }
      } else if (motion < 0.0 && entity.minY >= block.maxY - 1.0E-7) {
  ...
  private double clipXCollide(AABB entity, AABB block, double motion) {
    if (entity.maxY > block.minY && entity.minY < block.maxY && entity.maxZ > block.minZ && entity.minZ < block.maxZ) {
      if (motion > 0.0 && entity.maxX <= block.minX + 1.0E-7) {
        double gap = block.minX - entity.maxX;
        if (gap < motion) {
          motion = gap;
        }
      } else if (motion < 0.0 && entity.minX >= block.maxX - 1.0E-7) {
  ...
  private double clipZCollide(AABB entity, AABB block, double motion) {
    if (entity.maxX > block.minX && entity.minX < block.maxX && entity.maxY > block.minY && entity.minY < block.maxY) {
      if (motion > 0.0 && entity.maxZ <= block.minZ + 1.0E-7) {
        double gap = block.minZ - entity.maxZ;
        if (gap < motion) {
          motion = gap;
        }
      } else if (motion < 0.0 && entity.minZ >= block.maxZ - 1.0E-7) {
