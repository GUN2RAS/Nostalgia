...
public class AlphaMapGenCaves extends AlphaMapGenBase {
  public AlphaMapGenCaves() {
  }

  protected void generateCaveNode(
    int seedX, int seedZ, byte[] blocks, double x, double y, double z, float radius, float angle1, float angle2, int var12, int var13, double var14
  ) {
    double d4 = seedX * 16 + 8;
    double d5 = seedZ * 16 + 8;
    float f2 = 0.0F;
    float f3 = 0.0F;
    ...
    for (boolean flag1 = rand.nextInt(6) == 0; var12 < var13; var12++) {
      double d6 = 1.5 + AlphaMathHelper.sin(var12 * 3.1415927F / var13) * radius * 1.0F;
      double d7 = d6 * var14;
      float f4 = AlphaMathHelper.cos(angle2);
      float f5 = AlphaMathHelper.sin(angle2);
      x += AlphaMathHelper.cos(angle1) * f4;
      y += f5;
      z += AlphaMathHelper.sin(angle1) * f4;
      if (flag1) {
        angle2 *= 0.92F;
      } else {
        angle2 *= 0.7F;
      ...
      if (!flag && var12 == k1 && radius > 1.0F) {
        this.generateCaveNode(seedX, seedZ, blocks, x, y, z, rand.nextFloat() * 0.5F + 0.5F, angle1 - 1.5707964F, angle2 / 3.0F, var12, var13, 1.0);
        this.generateCaveNode(seedX, seedZ, blocks, x, y, z, rand.nextFloat() * 0.5F + 0.5F, angle1 + 1.5707964F, angle2 / 3.0F, var12, var13, 1.0);
        return;
      }
      ...
      if (flag || rand.nextInt(4) != 0) {
        double d8 = x - d4;
        double d9 = z - d5;
        double d10 = var13 - var12;
        double d11 = radius + 2.0F + 16.0F;
        ...
        if (x >= d4 - 16.0 - d6 * 2.0 && z >= d5 - 16.0 - d6 * 2.0 && x <= d4 + 16.0 + d6 * 2.0 && z <= d5 + 16.0 + d6 * 2.0) {
          ...
          if (!hitWater) {
            for (int k3 = l1; k3 < i2; k3++) {
              double d12 = (k3 + seedX * 16 + 0.5 - x) / d6;

              for (int i4x = l2; i4x < i3; i4x++) {
                double d13 = (i4x + seedZ * 16 + 0.5 - z) / d6;
                int j4 = (k3 * 16 + i4x) * 128 + k2;
                if (d12 * d12 + d13 * d13 < 1.0) {
                  for (int k4 = k2 - 1; k4 >= j2; k4--) {
                    double d14 = (k4 + 0.5 - y) / d7;
  ...
  protected void recursiveGenerate(AlphaLevelSource source, int chunkX, int chunkZ, int originX, int originZ, byte[] blocks) {
    ...
    for (int j = 0; j < i; j++) {
      ...
      if (this.rand.nextInt(4) == 0) {
        this.generateCaveNode(originX, originZ, blocks, d0, d1, d2, 1.0F + this.rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5);
      ...
      for (int l = 0; l < k; l++) {
        ...
        this.generateCaveNode(originX, originZ, blocks, d0, d1, d2, f2, f, f1, 0, 0, 1.0);
      }
    }
  }
}
