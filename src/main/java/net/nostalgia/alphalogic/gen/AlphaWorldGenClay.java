...
public class AlphaWorldGenClay {
  private final BlockState minableBlockState = Blocks.CLAY.defaultBlockState();
  private final int numberOfBlocks;

  public AlphaWorldGenClay(int amount) {
    this.numberOfBlocks = amount;
  }

  public boolean generate(WorldGenLevel level, Random random, int x, int y, int z) {
    MutableBlockPos pos = new MutableBlockPos();
    if (!level.getBlockState(pos.set(x, y, z)).is(Blocks.WATER)) {
      return false;
    } else {
              ...
              if (d12 * d12 + d13 * d13 < 1.0) {
                for (int i3 = k1; i3 <= j2; i3++) {
                  ...
                  if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0) {
                    pos.set(k2, l2, i3);
                    if (level.getBlockState(pos).is(AlphaBlocks.ALPHA_SAND)) {
