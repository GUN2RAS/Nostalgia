...
public class AlphaChunkDecorator {
  ...
  public static void decorate(ChunkPos cp, long seed) {
    ...
    currentRand.setSeed(chunkX * xSeed + chunkZ * zSeed ^ seed);
    ...
    double d2 = 0.5;
    int treeCount = (int)((treeNoise.generateNoise(startX * d2, startZ * d2) / 8.0 + currentRand.nextDouble() * 4.0 + 4.0) / 3.0);
    if (treeCount < 0) {
      treeCount = 0;
    }
