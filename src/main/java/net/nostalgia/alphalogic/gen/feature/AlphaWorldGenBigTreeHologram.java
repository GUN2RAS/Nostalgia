...
public class AlphaWorldGenBigTreeHologram {
        ...
        for(int k = -i; k <= i; ++k) {
            ...
            while(j <= i) {
                ...
                if (d0 > (double)size) {
                    ...
                    aint1[b1] = aint[b1] + j;
                    byte blockId = NostalgiaChunkCache.getBlockSafely(aint1[0], aint1[1], aint1[2]);
                    if (blockId != 0 && blockId != 18) {
                        ++j;
                    } else {
                        NostalgiaChunkCache.setBlockSafely(aint1[0], aint1[1], aint1[2], stateId);
                        ++j;
                    }
                }
            }
    ...
    void generateLeafNode(int x, int y, int z) {
        ...
        for(int j = y; j < i; ++j) {
            ...
            this.genTreeLayer(x, j, z, f, (byte)1, (byte)18); // 18 is Leaves
        }
    ...
    void generateTrunk() {
        ...
        this.placeBlockLine(aint, aint1, (byte) 17); // 17 is OAK_LOG
        if (this.trunkSize == 2) {
            ++aint[0];
            ++aint1[0];
    ...
    void generateLeafNodeBases() {
        ...
        for(int[] aint = new int[]{this.basePos[0], this.basePos[1], this.basePos[2]}; i < j; ++i) {
            ...
            if (this.leafNodeNeedsBase(k)) {
                this.placeBlockLine(aint, aint2, (byte) 17); // 17 is OAK_LOG
            }
        }
    }
    ...
    boolean validTreeLocation() {
        ...
        byte below = NostalgiaChunkCache.getBlockSafely(this.basePos[0], this.basePos[1] - 1, this.basePos[2]);
        if (below != 2 && below != 3) { // 2 = Grass, 3 = Dirt
            return false;
        } else {
