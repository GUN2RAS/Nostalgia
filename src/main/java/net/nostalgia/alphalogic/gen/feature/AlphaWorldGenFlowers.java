...
public class AlphaWorldGenFlowers {
    ...
    public boolean generate(Random rand, int x, int y, int z) {
        for (int i = 0; i < 64; ++i) {
            ...
            if (cy >= 0 && cy < 128 && AlphaByteCache.getBlockSafely(cx, cy, cz) == 0 && AlphaByteCache.getBlockSafely(cx, cy - 1, cz) == 2) {
                AlphaByteCache.setBlockSafely(cx, cy, cz, this.flowerId);
            }
        }
        return true;
    }
