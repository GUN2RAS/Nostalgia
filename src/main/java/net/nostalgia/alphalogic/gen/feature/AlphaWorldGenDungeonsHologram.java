    ...
    public boolean generate(Random random, int x, int y, int z) {
            ...
            for (int curY = y + height; curY >= y - 1; --curY) {
                for (int curZ = z - radiusZ - 1; curZ <= z + radiusZ + 1; ++curZ) {
                    if (curX == x - radiusX - 1 || curY == y - 1 || curZ == z - radiusZ - 1 || curX == x + radiusX + 1 || curY == y + height + 1 || curZ == z + radiusZ + 1) {
                        if (curY >= 0 && !isSolid(AlphaByteCache.getBlockSafely(curX, curY - 1, curZ))) {
                            AlphaByteCache.setBlockSafely(curX, curY, curZ, (byte) 0);
                            continue;
                        }
                        if (isSolid(AlphaByteCache.getBlockSafely(curX, curY, curZ))) {
        ...
        for (int chests = 0; chests < 2; ++chests) {
            for (int attempts = 0; attempts < 3; ++attempts) {
                ...
                if (AlphaByteCache.getBlockSafely(cx, cy, cz) != 0) continue;

                int adjacentSolid = 0;
                if (isSolid(AlphaByteCache.getBlockSafely(cx - 1, cy, cz))) ++adjacentSolid;
                if (isSolid(AlphaByteCache.getBlockSafely(cx + 1, cy, cz))) ++adjacentSolid;
                if (isSolid(AlphaByteCache.getBlockSafely(cx, cy, cz - 1))) ++adjacentSolid;
                if (isSolid(AlphaByteCache.getBlockSafely(cx, cy, cz + 1))) ++adjacentSolid;
