...
public class AlphaWorldGenLiquids {
  ...
  public boolean generate(WorldGenLevel level, Random rand, int x, int y, int z) {
    ...
    if (!level.getBlockState(pos.above()).is(AlphaBlocks.ALPHA_STONE)) {
      return false;
    } else if (!level.getBlockState(pos.below()).is(AlphaBlocks.ALPHA_STONE)) {
      return false;
    } else {
      BlockState current = level.getBlockState(pos);
      if (!current.isAir() && !current.is(AlphaBlocks.ALPHA_STONE)) {
        return false;
      } else {
        int solidFaces = 0;
        if (level.getBlockState(pos.west()).is(AlphaBlocks.ALPHA_STONE)) {
          solidFaces++;
        }

        if (level.getBlockState(pos.east()).is(AlphaBlocks.ALPHA_STONE)) {
          solidFaces++;
        }

        if (level.getBlockState(pos.north()).is(AlphaBlocks.ALPHA_STONE)) {
          solidFaces++;
        }

        if (level.getBlockState(pos.south()).is(AlphaBlocks.ALPHA_STONE)) {
          solidFaces++;
        }
