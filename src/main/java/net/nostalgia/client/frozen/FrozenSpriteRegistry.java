...
public final class FrozenSpriteRegistry {
  ...
  public static Identifier toFrozenId(Identifier live) {
    return Identifier.fromNamespaceAndPath("nostalgia", "frozen/" + live.getNamespace() + "/" + live.getPath());
  }
  ...
  public static void bindFrozenByLive() {
    ...
    for (Entry<Identifier, Identifier> e : LIVE_TO_FROZEN_ID.entrySet()) {
      TextureAtlasSprite frozen = FROZEN_BY_ID.get(e.getValue());
      if (frozen != null) {
        newMap.put(e.getKey(), frozen);
      }
    }
