...
public class ClientZoneTime {
    ...
    private static void maybeTrigger() {
        ...
        if (inZone != wasInZone) {
            if (net.nostalgia.client.events.core.ClientRitualEventRegistry.activeTransition() != null && !inZone) {
                wasInZone = inZone;
                return;
            }
    ...
    public static long getEffectiveClockTicks(long realClockTicks) {
        maybeTrigger();
        if (net.nostalgia.client.events.core.ClientRitualEventRegistry.activeTransition() != null && hasSnapshot) return snapshotClockTicks;
    ...
    public static long getEffectiveGameTime(long realGameTime) {
        maybeTrigger();
        if (net.nostalgia.client.events.core.ClientRitualEventRegistry.activeTransition() != null && hasSnapshot) return snapshotGameTime;
        boolean inZone = ClientFreezeRegions.isLocalPlayerInZone();
        float p = progress();
        if (p >= 1.0f) {
    ...
    public static float getEffectiveRain(float realRain) {
        maybeTrigger();
        if (net.nostalgia.client.events.core.ClientRitualEventRegistry.activeTransition() != null && hasSnapshot) return snapshotRain;
        boolean inZone = ClientFreezeRegions.isLocalPlayerInZone();
        float p = progress();
        if (p >= 1.0f) {
    ...
    public static float getEffectiveThunder(float realThunder) {
        maybeTrigger();
        if (net.nostalgia.client.events.core.ClientRitualEventRegistry.activeTransition() != null && hasSnapshot) return snapshotThunder;
        boolean inZone = ClientFreezeRegions.isLocalPlayerInZone();
        float p = progress();
        if (p >= 1.0f) {
        ...
        if (access != null) {
            ...
            if (!regions.isEmpty()) {
                ...
                if (old.chunkRadius() != newRadius) {
                    ...
                    net.nostalgia.network.NostalgiaClientNetworking.nostalgia$markZoneChunksDirty(net.minecraft.client.Minecraft.getInstance(), old.beaconPos(), 5);
                }
            }
        }
