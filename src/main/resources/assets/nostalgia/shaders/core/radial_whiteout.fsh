...
void main() {
    ...
    float whiteFactor = clamp(dist / outerRadius, 0.0, 1.0);
    // Темнота более светлая (светло-серо-голубой оттенок Эха)
    vec3 echoColor = vec3(0.55, 0.65, 0.75); 
    vec3 renderColor = mix(echoColor, vec3(1.0, 1.0, 1.0), whiteFactor);

    if (dist < innerRadius) {
        float distToEdge = innerRadius - dist;
        if (distToEdge < 4.0) {
            float glowAlpha = pow(1.0 - (distToEdge / 4.0), 2.0); 
            fragColor = vec4(renderColor, glowAlpha * globalFade);
        } else {
            fragColor = vec4(0.0);
        }
    } else if (dist < outerRadius) {
        if (absoluteWorldPos.y > BeaconPos.y) {
            fragColor = vec4(renderColor, globalFade);
        } else {
            ...
            if (waveAlpha > 0.01) {
                fragColor = vec4(renderColor, waveAlpha * globalFade);
            } else {
                fragColor = vec4(0.0);
            }
        }
